...
class FixedSizeQueue:
    def __init__(self, size=2):
        self.queue = deque(maxlen=size)  # 设置队列的最大长度为2

    def add(self, item):
        self.queue.append(item)  # 添加新元素到队列末尾
        #print(f"Added {item}, current queue: {list(self.queue)}")
...
check_for_error()
import chess_ai2 # only import if there's no error
...
def createSliders(): # creates sliders window
    ...
    cv2.createTrackbar("Y-Coord", "Sliders", 444, screen_height, nothing)
    cv2.createTrackbar("X-Offset", "Sliders", 843, screen_width, nothing)
    cv2.createTrackbar("Y-Offset", "Sliders", 843, screen_height, nothing)
    cv2.createTrackbar("Confidence", "Sliders", 50, 100, nothing)
    cv2.createTrackbar("W->B", "Sliders", 0, 1, nothing)
    cv2.createTrackbar("auto-move", "Sliders", 1, 1, nothing)
    cv2.createTrackbar("Off->On", "Sliders", 1, 1, nothing)
    cv2.createTrackbar("calc", "Sliders", 0, 1, nothing)
    cv2.createTrackbar("frist_step_ornot","Sliders",0,1, nothing)
    cv2.createTrackbar("today_frist_ornot","Sliders",0,1,nothing)
    cv2.createTrackbar("make_mistake", "Sliders", 60,100,  nothing)
    cv2.createTrackbar("win_or_not","Sliders",1,1,nothing)
    cv2.createTrackbar("opp_win_lose","Sliders",70,100,nothing)
...
def count_figures(figure_pos):
    i=0
    for key,value in  figure_pos.items():
        i=i+len(value)
    return i
...
createSliders()
fen_string_list=FixedSizeQueue()
fen_string_list.add("d")
fen_string_list.add("a")
num_list=FixedSizeQueue()
black_figures_pos_last=""
white_figures_pos_last="" #储藏上一次成功走出棋当时的对方的位置
black_kq="kq"
white_kq="KQ"
fen=""
start_time=time.time()
quanju.win_or_not=cv2.getTrackbarPos("win_or_not","Sliders")
while True:
        ...
        if img.size > 0:
            ...
            while cv2.getTrackbarPos("Off->On", "Sliders") == 1:
                ...
                if cv2.getTrackbarPos("calc", "Sliders") == 1:
                    ...
                    try:
                        if cv2.getTrackbarPos("frist_step_ornot","Sliders")==1:
                           xb,yb=next_turn.detecter("black\\Rooks.png","numpy.jpg")
                           xw,yw=next_turn.detecter("white\\Rooks.png","numpy.jpg")
                           cv2.setTrackbarPos("W->B", "Sliders",1) if yb>yw else  cv2.setTrackbarPos("W->B", "Sliders",0)
                           cv2.setTrackbarPos("frist_step_ornot","Sliders",0)
                           get_own_color()   
                        ...
                        get_figure_field()
                        num_black_figures_pos=count_figures(black_figures_pos)
                        num_white_figures_pos = count_figures(white_figures_pos)
                        #print(num_black_figures_pos)

                        
                        black_figures_pos_str=json.dumps(black_figures_pos)
                        white_figures_pos_str=json.dumps(white_figures_pos)
                        
                        #print(black_figures_pos_str)
                        #print(white_figures_pos_str)
                        '''
                        num_black_figures_pos=collections.Counter(black_figures_pos.values())
                        print(num_black_figures_pos)
                        num_white_figures_pos=collections.Counter(white_figures_pos.values())
                        print(num_white_figures_pos)'''
                        elapsed_time=time.time()-start_time
                        if own_color == "W": # sends the figures to the stackfish ai, to c1heck the best move
                            if chess_ai2.compare_fen(white_figures_pos, black_figures_pos,fen_string_list,num_list,num_black_figures_pos,black_figures_pos_last) and elapsed_time<=30:                #true为按兵不动继续循环，false为开始下棋
                                #print("fen_string是相同的")
                                #print(fen_string_list)
                                fen_string_list.add(black_figures_pos_str)                                                    #只要黑棋动了就开始,没有动就跳过
                                num_list.add(num_black_figures_pos)
                                
                                continue
                            else:
                                #if elapsed_time>30:
                                if next_turn.next_page(next_turn_pics,cv2.getTrackbarPos("today_frist_ornot","Sliders"),x,cv2.getTrackbarPos("opp_win_lose","Sliders")):
                                    cv2.setTrackbarPos("frist_step_ornot","Sliders",1)
                                    cv2.setTrackbarPos("today_frist_ornot","Sliders",0)
                                    x=False
                                    cv2.setTrackbarPos("win_or_not","Sliders",quanju.win_or_not)   #决定好下一局的胜负
                                else:
                                    cv2.setTrackbarPos("frist_step_ornot","Sliders",0)

                                fen,white_kq,black_kq=chess_ai2.fen_build(white_kq,black_kq,white_figures_pos,black_figures_pos,own_color)
                                start, end, fen_string = chess_ai2.get_best_move(white_figures_pos, black_figures_pos,fen,cv2.getTrackbarPos("make_mistake","Sliders"),quanju.win_or_not)     #  fen_string 用于存储当局的分，下一回合看有无变化。make_mistake是指选择非最佳选择的机率
                                fen_string_list.add(black_figures_pos_str)
                                num_list.add(num_black_figures_pos)
                                black_figures_pos_last=black_figures_pos_str
                                print("开始下棋")
                        else:
                            if chess_ai2.compare_fen(black_figures_pos, white_figures_pos,fen_string_list,num_list,num_white_figures_pos,white_figures_pos_last) and elapsed_time<=30:                #
                                #print("fen_string是相同的")
                                fen_string_list.add(white_figures_pos_str)
                                num_list.add(num_white_figures_pos)
                                ...
                                if next_turn.next_page(next_turn_pics,cv2.getTrackbarPos("today_frist_ornot","Sliders"),x,cv2.getTrackbarPos("opp_win_lose","Sliders")):
                                    ...
                                    cv2.setTrackbarPos("today_frist_ornot","Sliders",0)
                                    x=False
                                    cv2.setTrackbarPos("win_or_not","Sliders",quanju.win_or_not)
                                else:
                                    cv2.setTrackbarPos("frist_step_ornot","Sliders",0)
                                fen, black_kq,white_kq= chess_ai2.fen_build(black_kq,white_kq, black_figures_pos,white_figures_pos,own_color)
                                start, end ,fen_string= chess_ai2.get_best_move(black_figures_pos, white_figures_pos,fen,cv2.getTrackbarPos("make_mistake","Sliders"),quanju.win_or_not)
                                fen_string_list.add(white_figures_pos_str)
                                num_list.add(num_white_figures_pos)
                                white_figures_pos_last=white_figures_pos_str
                                print("开始下棋")
                        ...
                        if cv2.getTrackbarPos("auto-move", "Sliders") == 1:
                            x=True if next_turn.detecter("numpy.jpg","xx.jpg") else False  #保存这一局的状态，是否结束
                            pg.moveTo(mon["left"] + x + fields[start][0], mon["top"] + y + fields[start][1],duration=0.1)
                            pg.click(mon["left"] + x + fields[start][0], mon["top"] + y + fields[start][1],duration=0.1)
                            time.sleep(random.randint(1,2))
                            #pg.dragTo(mon["left"] + x + fields[end][0], mon["top"] + y + fields[end][1], button='left')
                            pg.moveTo(mon["left"] + x + fields[end][0], mon["top"] + y + fields[end][1],duration=0.1)
                            pg.click(mon["left"] + x + fields[end][0], mon["top"] + y + fields[end][1],duration=0.1)
                        ...
                        #cv2.setTrackbarPos("calc", "Sliders", 0) #改为自动
